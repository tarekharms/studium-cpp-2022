...
int xPosForStepNum(int step){
  int xspace = 8;
  if (!printSource) {
    xspace = 5;
  }
  return 1+xspace+step*2;
}
...
void printCommands(std::string fileName){
  ...
  for(unsigned int i=0; i<commands.size();i++){
    if(commands[i].line>0){
      std::string stepNumStr =  std::to_string(commands[i].stepNum);
      ...
      ansiConsole.printText(1+i*2+xspace, y, stepNumStr, colorToUse, true);
